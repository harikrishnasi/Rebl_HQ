...
drop policy if exists "leads select own" on leads;
...
drop policy if exists "task_completions delete own" on task_completions;
...
drop policy if exists "document_versions delete own" on document_versions;
...
drop policy if exists "settings insert own" on settings;
create policy "settings insert own" on settings for insert to authenticated with check (user_id = auth.uid());
drop policy if exists "settings update own" on settings;
...
drop policy if exists "settings delete own" on settings;
...
drop policy if exists "hq-docs read own" on storage.objects;
...
drop policy if exists "hq-docs insert own" on storage.objects;
...
drop policy if exists "hq-docs update own" on storage.objects;
create policy "hq-docs update own" on storage.objects for update to authenticated
  using (bucket_id = 'hq-docs' and (storage.foldername(name))[1] = auth.uid()::text)
  with check (bucket_id = 'hq-docs' and (storage.foldername(name))[1] = auth.uid()::text);
drop policy if exists "hq-docs delete own" on storage.objects;
